...
tdata="ring"
 ...
 dm1 <- fread(paste('data/p2_ring_updated_20190206.csv',sep=''),data.table=F,na.strings=c(NA,"-9"))
 dm1 <- subset(dm1,!is.na(Current_PTSD_Continuous_T2) | !is.na(Current_PTSD_Continuous_T1))
 
 
 dm1$Current_PTSD_Continuous_whichmax <- apply(dm1[,c("Current_PTSD_Continuous_T2","Current_PTSD_Continuous_T1")],1,which.max)

 takecol <- function(x)
 {
  column_to_pick <- as.numeric(x[3])
  return(x[column_to_pick])
  print(x[column_to_pick])
 }
 dm1$Current_PTSD_Continuous <- apply(dm1[,c("Current_PTSD_Continuous_T2","Current_PTSD_Continuous_T1","Current_PTSD_Continuous_whichmax")],1,takecol)
 ...
 for (ancgroup in c("eur"))
 {
  ...
  write.table(dm1_exp,file=paste("pheno/p2_",tdata,"_ring","_",ancgroup,".pheno",sep=""),quote=F,row.names=F)
    
  dm_cov <- dm1x  

  covexp4 <- subset(dm_cov,select=c("FID","IID","C1","C2","C3","C4","C5")) #mrs doesnt need sex!
  
  write.table(covexp4,file=paste("pheno/p2_",tdata,"_",ancgroup,"_ring","_pcs.cov",sep=""),quote=F,row.names=F)
 }  
