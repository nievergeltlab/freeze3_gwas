...
# dm1 <- fread(paste('data/p2_brya_UPDATED_20190206.csv',sep=''),data.table=F,na.strings=c(NA,"-9"))

 #dm1 <- remove_empty(dm1, which = c("cols"))

 fam <- fread(paste('/home/pgcdac/DWFV2CJb8Piv_0116_pgc_data/pts/wave3/v1/',tdata,'/qc/pts_',tdata,'_mix_am-qc.fam',sep=''),data.table=F,na.strings=c("NA","-9"))
 ...
 for (ancgroup in c("eur"))
 {
  ...
  dm1x <- dma # <- merge(dm1,dma,by=c("FID","IID"),suffixes=c("_fam","")) 
  ...
  write.table(dm1_exp,file=paste("pheno/p2_",tdata,"_brya","_",ancgroup,".pheno",sep=""),quote=F,row.names=F)
    
  dm_cov <- dm1x  

  covexp4 <- subset(dm_cov,select=c("FID","IID","C1","C2","C3","C4","C5")) #mrs doesnt need sex!
  
  write.table(covexp4,file=paste("pheno/p2_",tdata,"_",ancgroup,"_brya","_pcs.cov",sep=""),quote=F,row.names=F)
 }  
