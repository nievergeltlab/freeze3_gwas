...
tdata="pts1"
 ...
 dm1 <- fread(paste('data/p2_port_updated_20190123.csv',sep=''),data.table=F,na.strings=c(NA,"-9"))
 dm1 <- remove_empty(dm1, which = c("cols"))
 
 dm1$Current_PTSD_Continuous  <- dm1$Severity_PTSD_Murcia_PT_NoRandom
 dm1 <- remove_empty(dm1, which = c("cols"))
 ...
 dm1x$Case <- dm1x$Lifetime_PTSD_Dx
  ...
  write.table(dm1_exp,file=paste("pheno/p2_",tdata,"_port","_",ancgroup,".pheno",sep=""),quote=F,row.names=F)
    
  dm_cov <- dm1x  

  covexp4 <- subset(dm_cov,select=c("FID","IID","C1","C2","C3","C4","C5")) #mrs doesnt need sex!
  
  write.table(covexp4,file=paste("pheno/p2_",tdata,"_",ancgroup,"_port","_pcs.cov",sep=""),quote=F,row.names=F)
 }  
