...
tdata="comc"
 ...
 dm1 <- fread(paste('data/p2_com1_updated_20190313.csv',sep=''),data.table=F,na.strings=c(NA,"-9"))
 dm1 <- remove_empty(dm1, which = c("cols"))
 

  
 dm1$Lifetime_PTSD_Dx <- apply(dm1[,c("Lifetime_cil_PTSD_Dx","Lifetime_mil_PTSD_Dx")],1,max,na.rm=T)
 dm1$Current_PTSD_Dx <- apply(dm1[,c("Current_cil_PTSD_Dx","Current_mil_PTSD_Dx")],1,max,na.rm=T)
 ...
 for (ancgroup in c("eur","aam"))
 {
  ...
  write.table(dm1_exp,file=paste("pheno/p2_",tdata,"_comc","_",ancgroup,".pheno",sep=""),quote=F,row.names=F)
    
  dm_cov <- dm1x  

  covexp4 <- subset(dm_cov,select=c("FID","IID","C1","C2","C3","C4","C5")) #mrs doesnt need sex!
  
  write.table(covexp4,file=paste("pheno/p2_",tdata,"_",ancgroup,"_comc","_pcs.cov",sep=""),quote=F,row.names=F)
 }  
